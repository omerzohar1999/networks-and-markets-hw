...
\begin{enumerate}[label=(\alph*)]
    ...
    Unsurprisingly, the GSP prices are also increasing with the valuation of the bidders for the singular good (up to tie-breaks), as for every bidder $b_i$, the price/item is set to be the valuation of the next bidder $b_{i+1}$ (or $b_i$ if $i = n$), and the larger bundles are assigned to the higher-valuing bidders (as in the VCG mechanism). Thus, the GSP prices are increasing with the valuation of the bidders for the singular good, as the VCG prices are.

    \paragraph{Contexts in which VCG and GSP prices are similar}
    FILL IN 

    \paragraph{Contexts in which VCG and GSP prices are different}
    FILL IN
